    ...
    overrides: [
      {
        ...
        // A function, not a RegExp: SDK 57's @expo/metro-config loads this
        // config without a filename to build its transform cache key, and
        // Babel rejects string/RegExp patterns in that case.
        include: (filename) =>
          typeof filename === 'string' && /node_modules\/@fastshot\/(ai|auth)/.test(filename),
        plugins: [
          [
